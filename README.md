...
本仓库的核心功能是提供一个**通过 Excel 表格驱动的 For 循环工作流**，用于批量生成图片作为 Lora 训练集，并自动生成对应的原生 Tagger 的txt文本文件用于打标。（后者功能有分类，且可以禁用，单纯roll图也没有问题）除此之外，你可以用固定的提示词来批量测试lora。
对了，它最大的一个功能就是断点。允许接上先前暂停执行的部分（因为是用excel存储的数据）。
我还提供了对词典的各种操作。

### 有部分节点可能因为升级出问题。在节点右键刷新一下就行。
...
这个工作流能够自动分类出带有工作流的图像，所以你不必去一张张翻找带有工作流的图像了。

在分类完后，你能使用这个工作流对a1111文件进行一个自动的读取。

你可以将这部分工作流复制下来，黏贴到你原来的工作流中去，你就获得了一个以图像为词典（抽卡，或者按某种顺序读取）的工作流了。

这个工作流可以自动分类出没有基本信息的图像。然后他们可以被自动反推，并注入正面提示词到元数据（A1111）并保存。

同时，这个工作流依旧带有抽卡功能（将按序号读取节点的排序选项改为随机），意味着你能将图像作为词典。

## 我发现我超会循环的工作流，如果你有什么定制需求务必联系我（发到issues）。
