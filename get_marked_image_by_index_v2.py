# ComfyUI-AutoData-for-lora/get_marked_image_by_index_v4.py
import os
import re
import json
import datetime
...
def _自然排序键(s: str, _nsre=re.compile('([0-9]+)')) -> List[Any]:
    """
    提供自然排序的键 (例如, "item2" 会在 "item10" 之前)。
    """
    return [int(text) if text.isdigit() else text.lower() for text in _nsre.split(s)]

def _pil转张量(图像: Image.Image) -> torch.Tensor:
    """
    将 PIL.Image 对象转换为符合 ComfyUI 标准的 PyTorch 张量。
    """
    图像_np = np.array(图像).astype(np.float32) / 255.0
    if len(图像_np.shape) == 2:
        图像_np = np.expand_dims(图像_np, axis=2)
    张量 = torch.from_numpy(图像_np)
    if 张量.dim() == 3 and 张量.shape[2] == 1:
        张量 = 张量.repeat(1, 1, 3)
    if 张量.shape[2] == 4:
        张量 = 张量[:, :, :3]
    张量 = 张量.unsqueeze(0)
...
class 按序号加载标记图像_V4:
    """
    按序号加载标记图像 V4 - 在指定文件夹中筛选并排序文件，
    加载特定序号的图像，并输出包含文件信息和嵌入提示的完整JSON元数据。
    """
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "搜索标记": ("STRING", {"default": "", "multiline": False, "placeholder": "查找包含此符号的文件"}),
                "排除标记": ("STRING", {"default": "", "multiline": False, "placeholder": "排除包含此符号的文件"}),
    ...
    def 加载图像(self, 文件夹路径: str, 序号: int, 排序方式: str, 文件扩展名: str, 搜索标记: str, 排除标记: str, 从名称中移除搜索标记: bool):
        print(f"\n[{self.节点名称}] 节点开始执行...")
        print(f"  > 参数: 文件夹='{文件夹路径}', 序号={序号}, 排序='{排序方式}', 搜索='{搜索标记}', 排除='{排除标记}'")
        ...
        try:
            ...
            # --- [新] 复合元数据提取逻辑 ---
            # 1. 提取文件系统信息
            file_size_bytes = os.path.getsize(待加载的完整路径)
            mod_time_stamp = os.path.getmtime(待加载的完整路径)
            
            fileinfo = {
                "filename": 待加载的完整路径,
                "resolution": f"{pil_图像.width}x{pil_图像.height}",
                "date": datetime.datetime.fromtimestamp(mod_time_stamp).strftime("%Y-%m-%d %H:%M:%S"),
                "size": self._格式化文件大小(file_size_bytes)
            }

            # 2. 提取嵌入的参数元数据 (如 ComfyUI/A1111 prompts)
            embedded_params = ""
            if pil_图像.info:
                if 'parameters' in pil_图像.info:
                    embedded_params = pil_图像.info['parameters']
                elif 'prompt' in pil_图像.info:
                    embedded_params = pil_图像.info['prompt']
            ...
            print(f"[{self.节点名称}] 已提取并格式化完整元数据。")
            # --- 元数据提取逻辑结束 ---
...
# --- 节点注册 ---
NODE_CLASS_MAPPINGS = {
    "GetMarkedImageByIndex_AutoData_V4_CN": 按序号加载标记图像_V4
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "GetMarkedImageByIndex_AutoData_V4_CN": "按序号加载标记图像 V4 [自动数据]"
}